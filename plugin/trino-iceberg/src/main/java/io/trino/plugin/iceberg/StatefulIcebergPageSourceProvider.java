...
public class StatefulIcebergPageSourceProvider
        implements ConnectorPageSourceProvider
{
            ...
            Optional<NameMapping> nameMapping)
    {
        ...
        Optional<ReaderProjectionsAdapter> projectionsAdapter = dataPageSource.getReaderColumns().map(readerColumns ->
                new ReaderProjectionsAdapter(
                        ...
                        StatefulIcebergPageSourceProvider::applyProjection));
        ...
        Supplier<Optional<RowPredicate>> deletePredicate =
                Suppliers.memoize(() -> getDeleteManager(partitionSpec, partitionData).getDeletePredicate(tableSchema,
                        ...
                        readColumns,
                        typeManager,
                        (deleteFile, deleteColumns, tupleDomain) -> openDeletes(session, deleteFile, deleteColumns, tupleDomain)));
            ...
            implements OrcReader.FieldMapperFactory
    {
        ...
        public OrcReader.FieldMapper create(OrcColumn column)
        {
            Map<Integer, OrcColumn> nestedColumns = uniqueIndex(
                    ...
                    StatefulIcebergPageSourceProvider::getIcebergFieldId);
            ...
            List<IcebergColumnHandle> columns)
    {
        ...
        try (DataFileStream<?> avroFileReader = new DataFileStream<>(file.newStream(), new GenericDatumReader<>())) {
            ...
            if (nameMapping.isPresent() && fileFields.stream().noneMatch(StatefulIcebergPageSourceProvider::hasId)) {
    ...
    private static class DereferenceChain
    {
        ...
        public Iterable<DereferenceChain> orderedPrefixes()
        {
            return () -> new AbstractIterator<>()
            {
                ...
                @Override
                public StatefulIcebergPageSourceProvider.DereferenceChain computeNext()
                {
                    if (prefixLength > path.size()) {
                        return endOfData();
                    }
                    return new StatefulIcebergPageSourceProvider.DereferenceChain(baseColumnIdentity, path.subList(0, prefixLength++));
                }
            };
        }
