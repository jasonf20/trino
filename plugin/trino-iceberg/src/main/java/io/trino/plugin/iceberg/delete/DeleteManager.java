...
public class DeleteManager
{
            ...
            boolean forceWait)
    {
        ...
        EqualityDeleteFilter.FileToken fileToken = equalityDeleteFilter.getLoadFileToken(delete.path(), delete.getDataSequenceNumber());
        if (fileToken != null) { // If the file load token is null the file is already loaded
            boolean shouldLoad = fileToken.startLoading();
            if (!shouldLoad && !forceWait) {
                ...
                synchronized (fileToken) {
                    if (!fileToken.isLoaded()) {
                        try (ConnectorPageSource pageSource = deletePageSourceProvider.openDeletes(delete, columns, TupleDomain.all())) {
                            equalityDeleteFilter.loadEqualityDeletes(pageSource, columns, delete.getDataSequenceNumber());
                            fileToken.setLoaded();
                        }
                        catch (IOException e) {
                            fileToken.resetLoading();
                            throw new UncheckedIOException(e);
                        }
                    }
                }
    ...
    private EqualityDeleteFilter getFilterByDeleteSchema(Schema deleteSchema, List<Integer> equalityDeleteFieldIds)
    {
        synchronized (equalityDeleteFiltersBySchema) {
            return equalityDeleteFiltersBySchema.computeIfAbsent(ImmutableSet.copyOf(equalityDeleteFieldIds),
                    (s) -> new EqualityDeleteFilter(deleteSchema, this));
        }
    }
}
